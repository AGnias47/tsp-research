...
I have the repo in a usable state. There are a few ways to get it running.

* At this Google Drive link, I've zipped the repo and all dependencies. If you unzip it on a Linux server and run `source .venv/bin/activate`, it should be ready to run.
* You can also clone the repository from GitHub via `git clone https://github.com/jvkersch/pyconcorde.git`. Then you install dependencies
  * Install Python 3.13
  * Install dependencies with `python -m pip install -r requirements.txt`
  * Install pyconcorde with `python -m pip install 'pyconcorde @ git+https://github.com/jvkersch/pyconcorde'`

This would have to be done on a Linux OS, or on Windows WSL. If you can only run on Windows, let me know and I can make some quick adjustments.

## Contents of the repo

### Problem Data

Problems I am currently working with for testing are in the `data` directory. `barachet10` has been a good medium size test problem, and `berlin52` is a good larger one. Results that have been tracked so far are in `results.csv`.
...
Everything runs through the `main.py` script. The script takes 2 arguments. All availabe options can be seen by running `python main.py --help`.
